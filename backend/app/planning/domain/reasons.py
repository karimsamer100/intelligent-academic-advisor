"""Stable machine-readable planning reason and review codes."""

from enum import StrEnum


class ReasonCode(StrEnum):
    """Initial reason vocabulary shared by future planning results."""

    MISSING_PREREQUISITE = "MISSING_PREREQUISITE"
    ALREADY_COMPLETED = "ALREADY_COMPLETED"
    CURRENTLY_REGISTERED = "CURRENTLY_REGISTERED"
    WRONG_REGULATION = "WRONG_REGULATION"
    WRONG_PROGRAM = "WRONG_PROGRAM"
    UNVERIFIED_RULE = "UNVERIFIED_RULE"
    UNAPPROVED_RULE = "UNAPPROVED_RULE"
    CONFLICTED_RULE = "CONFLICTED_RULE"
    BLOCKED_RULE = "BLOCKED_RULE"
    MISSING_REQUIRED_DATA = "MISSING_REQUIRED_DATA"
    UNSUPPORTED_RULE = "UNSUPPORTED_RULE"
    UNSUPPORTED_CASE = "UNSUPPORTED_CASE"
    EXCEPTION_REQUIRED = "EXCEPTION_REQUIRED"
    AMBIGUOUS_REGULATION = "AMBIGUOUS_REGULATION"
    CONDITIONAL_REQUIREMENT = "CONDITIONAL_REQUIREMENT"
    ADVISOR_REVIEW_REQUIRED = "ADVISOR_REVIEW_REQUIRED"
    INTENT_MISMATCH = "INTENT_MISMATCH"
    CONCURRENT_CONTEXT_INVALID = "CONCURRENT_CONTEXT_INVALID"
    ENTRY_REQUIREMENT_UNRESOLVED = "ENTRY_REQUIREMENT_UNRESOLVED"
    DUPLICATE_COURSE = "DUPLICATE_COURSE"
    UNKNOWN_COURSE_CREDITS = "UNKNOWN_COURSE_CREDITS"
    LOAD_POLICY_UNRESOLVED = "LOAD_POLICY_UNRESOLVED"
    LOAD_LIMIT_EXCEEDED = "LOAD_LIMIT_EXCEEDED"
    CANDIDATE_COVERAGE_INCOMPLETE = "CANDIDATE_COVERAGE_INCOMPLETE"
    CANDIDATE_COVERAGE_UNAVAILABLE = "CANDIDATE_COVERAGE_UNAVAILABLE"
    PLAN_TARGET_NOT_REACHED = "PLAN_TARGET_NOT_REACHED"
    PLAN_SEARCH_LIMIT_REACHED = "PLAN_SEARCH_LIMIT_REACHED"
    PROJECTION_ASSUMPTION = "PROJECTION_ASSUMPTION"
    PROJECTION_UNCERTAIN = "PROJECTION_UNCERTAIN"
    NO_PROGRESS = "NO_PROGRESS"
    HORIZON_REACHED = "HORIZON_REACHED"
    OFFERING_UNAVAILABLE = "OFFERING_UNAVAILABLE"
    TIMETABLE_UNAVAILABLE = "TIMETABLE_UNAVAILABLE"
    SCENARIO_EXCLUSION = "SCENARIO_EXCLUSION"
    UEL_MODULE_FAILED = "UEL_MODULE_FAILED"
    UEL_MODULE_OUTSTANDING = "UEL_MODULE_OUTSTANDING"
    UEL_MODULE_IN_PROGRESS = "UEL_MODULE_IN_PROGRESS"
    UEL_PROGRESS_UNKNOWN = "UEL_PROGRESS_UNKNOWN"
    UEL_PROGRESS_INCOMPLETE = "UEL_PROGRESS_INCOMPLETE"
    UEL_PROGRESS_UNVERIFIED = "UEL_PROGRESS_UNVERIFIED"
    UEL_MAPPING_INCOMPLETE = "UEL_MAPPING_INCOMPLETE"
    UEL_MAPPING_UNSAFE = "UEL_MAPPING_UNSAFE"
    UEL_PROGRESSION_RISK = "UEL_PROGRESSION_RISK"
    UEL_REVIEW_REQUIRED = "UEL_REVIEW_REQUIRED"
